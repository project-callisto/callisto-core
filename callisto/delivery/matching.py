...
def find_matches(report_class=PDFMatchReport):
    ...
    new_identifiers = MatchReport.objects.filter(seen=False).order_by(
        'identifier').values('identifier').distinct()
    for row in new_identifiers.values():
        ...
        if len(match_list) > 1:
            ...
            new_match_owners = [
                match.report.owner for match in match_list if not match.seen]
