...
# TODO: customize
# https://github.com/SexualHealthInnovations/callisto-core/issues/20
...
class PDFReport(object):
    ...
    def format_question(self, question, strikethrough=False):
        markup_open, markup_close = (
            '<strike>', '</strike>') if strikethrough else ('', '')
        return [
            Paragraph(
                markup_open +
                question +
    ...
    # TODO: figure out how to keep question w answer
    # https://github.com/SexualHealthInnovations/callisto-core/issues/34
    def add_multiple_choice(
            self,
            question,
            answers,
            selected_answers,
            last_is_free_text=False):
        self.add_question(question)

        def build_list_item(index, text):
            markup_open, markup_close = (
                '<b>', '</b>') if (index in selected_answers) else ('', '')
            return self.format_answer(
                (markup_open +
                 text +
                 markup_close),
                self.free_text if last_is_free_text and (
                    index == len(answers) -
                    1) else self.selected if (
                    index in selected_answers) else self.unselected)

        self.add_answer_list([build_list_item(i, t)
    ...
    def render_question(self, question):
        ...
        if question_type == 'RadioButton' or question_type == 'Checkbox':
            ...
            # RadioButton answers need to be stored as single int to keep edit
            # working
            ...
            if extra:
                choices.append(
                    "<i>{0}</i>: {1}".format(extra.get('extra_text'), extra.get('answer')))
            ...
            answer = question.get('answer').replace(
                '\n', '<br />\n') or '<i>Not answered</i>'
            self.add_answer_list([self.format_answer(
                answer, self.free_text)], keep_together=False)
    ...
    def send_email_to_coordinator(
        ...
        # https://github.com/SexualHealthInnovations/callisto-core/issues/32
        attachment = gpg.encrypt(
            pdf_to_attach,
    ...
    def get_metadata_page(self, recipient):
        ...
        if recipient:
            overview_body = overview_body + "Submitted on:  {0}<br/>".format(
                localtime(self.report.submitted_to_school) .strftime(date_format))
        ...
        for section_id, section_name in PageBase.SECTION_CHOICES:
            self.pdf_elements.append(
                Paragraph(
                    ...
                    self.section_title_style))
            section_qs = [
                x for x in report_content if x.get('section') == section_id]
            for q in section_qs:
                self.render_question(q)
    ...
    def generate_match_report(self, report_id):
        ...
        self.pdf_elements.append(
            Paragraph(
                "Perpetrator",
                self.section_title_style))
        names = ', '.join(set([match.name.strip()
                               for match in self.matches if match.name]))
        if len(names) < 1:
            names = '<i>None provided</i>'
        perp_info = ('Name(s): ' + names) + '<br/>' + \
            self.matches[0].identifier
        self.pdf_elements.append(Paragraph(perp_info, self.body_style))

        for idx, match in enumerate(
                sorted(self.matches, key=lambda match: match.added, reverse=True)):
            self.pdf_elements.append(
                Paragraph("Report " + str(idx + 1), self.section_title_style))
            ...
            overview_body = """Perpetrator name given: {0}
                ...
                match.name or "<i>None provided</i>",
            ...
            contact_body = """<br/>Name: {0}
                              ...
                              Email: {3}
                              Notes on preferred contact time of day, gender of admin, etc.:""".format(
                match.contact_name or "<i>None provided</i>",
                match.contact_phone,
            ...
            self.pdf_elements.append(
                Paragraph(
                    match.contact_notes or "None provided",
                    self.notes_style))
