...
class SingleLineText(RecordFormItem):
    ...
    def serialize_for_report(self, answer = {}, *args):
        return {'id': self.pk, 'question_text': self.text, 'answer': conditional_escape(answer), 'type': 'SingleLineText', 'section': self.section}
...
class MultiLineText(RecordFormItem):
    ...
    def serialize_for_report(self, answer = {}, *args):
        return {'id': self.pk, 'question_text': self.text, 'answer': conditional_escape(answer), 'type': 'MultiLineText', 'section': self.section}
...
class RadioButton(MultipleChoice):
    ...
    def make_field(self):
        ...
        return forms.ChoiceField(choices = choice_tuples,
                                 label=self.text,
                                 required=False,
                                 widget= forms.Select(attrs={'class': "form-control input-lg"}) if self.is_dropdown else forms.RadioSelect)
    ...
    def get_extras(self):
        ...
        return [("question_%s_extra-%s" % (self.pk, choice.pk), choice.extra_info_placeholder) for choice in choices if choice.extra_info_placeholder]
...
class Date(RecordFormItem):
    ...
    def serialize_for_report(self, answer, *args):
        return {'id': self.pk, 'question_text': self.text, 'answer': conditional_escape(answer), 'type': 'Date', 'section': self.section}
