...
class ReportDeliveryTest(MatchTest):
    ...
    def test_pdf_full_report_is_generated(self):
        report = PDFFullReport(self.user, self.report, self.decrypted_report)
        output = report.generate_pdf_report(toname=None, report_id=None)
        exported_report = BytesIO(output)
        pdfReader = PyPDF2.PdfFileReader(exported_report)
        self.assertIn("Callisto", pdfReader.getPage(0).extractText())
        self.assertIn("Submitted by: dummy", pdfReader.getPage(1).extractText())
        self.assertIn("test answer", pdfReader.getPage(2).extractText())
        self.assertIn("answer to 2nd question", pdfReader.getPage(2).extractText())
    ...
    def test_submission_to_school(self):
        ...
        self.assertEqual(message.attachments[0][0], 'callisto_report_%s.pdf.gpg' % sent_report_id)
    ...
    def test_pdf_match_report_is_generated(self):
        ...
        self.assertIn("Callisto", pdfReader.getPage(0).extractText())
        self.assertIn("Submitted by: dummy", pdfReader.getPage(1).extractText())
        self.assertIn("Submitted by: ymmud", pdfReader.getPage(1).extractText())
