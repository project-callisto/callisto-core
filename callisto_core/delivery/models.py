...
class Report(models.Model):
    ...
    def encrypt_record(
        self,
        record_data_string: str,  # the record questions, as a string of json
        passphrase: str,
    ) -> None:
        '''Encrypts and saves record data, in two formats'''
        record_data_json = json.dumps(record_data_string)
        self._store_for_user_decryption(record_data_json, passphrase)
        self._store_for_callisto_decryption(record_data_json)
        self.save()
    ...
    def _store_for_user_decryption(self, record_data_json, passphrase):
        '''
        store user decryptable data and 500 the request on fails
        '''
        stretched_key = self.encryption_setup(passphrase)
        self.encrypted = security.encrypt_text(stretched_key, record_data_json)

    def _store_for_callisto_decryption(self, record_data_json):
        '''
        store callisto decryptable data and ignore fails
        filters out skip_eval fields before storing
        '''
        try:
            filtered_data = model_helpers.filter_record_data(record_data_json)
...
class MatchReport(models.Model):
    ...
    def encrypt_match_report(
        self,
        record_data_string: str,  # MatchReportContent as a string of json
        ...
        self.encrypted = security.pepper(
            security.encrypt_text(stretched_identifier, record_data_string),
        )
        self.save()
