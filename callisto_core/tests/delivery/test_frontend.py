...
class EncryptedFrontendTest(AuthMixin, AssertionsMixin, StaticLiveServerTestCase):
    ...
    def setup_data(cls):
        call_command(
            "loaddata", *cls.fixtures, **{"verbosity": 1, "database": "default"}
        )
...
class WizardFrontendCases(EncryptedFrontendTest):
    ...
    def test_second_page_text(self):
        self.element.next.click()
        self.assertSelectorContains("form", "do androids dream of electric sheep?")
    ...
    def test_textbox_array_regression(self):
        ...
        self.element.next.click()
        self.assertNotEqual(
            ["text input content!!!"], self.element.text_input.get_attribute("value")
        )
