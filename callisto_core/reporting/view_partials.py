...
class _SubmissionPartial(
    ...
    def coordinator_emails(self):
        return TenantApi.site_settings("COORDINATOR_EMAIL", request=self.request)

    @property
    def coordinator_public_key(self):
        return TenantApi.site_settings("COORDINATOR_PUBLIC_KEY", request=self.request)

    @property
    def school_name(self):
        return TenantApi.site_settings("SCHOOL_SHORTNAME", request=self.request)

    @property
    def school_email_domain(self):
        return TenantApi.site_settings("SCHOOL_EMAIL_DOMAIN", request=self.request)
...
class SchoolEmailConfirmationPartial(SchoolEmailFormPartial):
...
class PrepPartial(_SubmissionPartial):
    form_class = forms.PrepForm
    EVAL_ACTION_TYPE = "CONTACT_INFO_PREPERATION"


class ResubmitPrepPartial(PrepPartial):
...
class _MatchingPartial(_ReportSubclassPartial):
    def form_valid(self, form):
...
class RequiredMatchingPartial(_MatchingPartial):
    EVAL_ACTION_TYPE = "ENTER_MATCHING_REQUIRED"
    form_class = forms.MatchingRequiredForm


class ConfirmationPartial(_ReportSubclassPartial):
...
class ResubmitConfirmationPartial(ConfirmationPartial):
    form_class = forms.ConfirmedConfirmationForm
    EVAL_ACTION_TYPE = "RESUBMIT_FINAL_CONFIRMATION"
    SLACK_ALERT_TEXT = (
        "Resubmitted Callisto Report at {school_name} (details will be sent via email)"
    )


class MatchingWithdrawPartial(
    view_helpers.ReportingSuccessUrlMixin, delivery_partials._ReportActionPartial
):
