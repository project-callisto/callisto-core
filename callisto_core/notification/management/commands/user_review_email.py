...
class UserReviewCommandBackend(object):
    ...
    def reports(self):
        return Report.objects.filter(
            owner__account__invalid=False, sentfullreport__to_address__isnull=False
        )

    @property
    def matches(self):
        return MatchReport.objects.filter(
            report__match_found=True, report__owner__account__invalid=False
        )
    ...
    def send_user_review_email(self):
        NotificationApi.send_user_review_nofication(
            ...
            to_addresses=[
                TenantApi.site_settings("COORDINATOR_EMAIL", site_id=self.site_id)
            ],
            public_key=TenantApi.site_settings(
                "COORDINATOR_PUBLIC_KEY", site_id=self.site_id
            ),
...
class Command(UserReviewCommandBackend, BaseCommand):
