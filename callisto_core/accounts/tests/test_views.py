...
class SignupViewIntegratedTest(AccountsTestCase):
    ...
    def test_redirects_to_next(self):
        response = self.client.post(
            self.signup_url + "?next=" + reverse("report_new"), self.DEFAULT_POST
        )
    ...
    def test_accepts_optional_email(self):
        ...
        self.assertEqual(User.objects.get(username="test").email, "test@email.co.uk")
    ...
    def test_sets_site_id(self):
        ...
        with TempSiteID(temp_site_id):
            ...
            self.assertEqual(
                User.objects.get(username="test").account.site_id, temp_site_id
            )
...
class StudentVerificationTest(AccountsTestCase):
    def setUp(self):
        ...
        Account.objects.create(
            user=self.user, site_id=1, school_email="tech@projectcallisto.org"
        )
    ...
    def test_verification_get(self):
        ...
        self.assertIsInstance(response.context["form"], ReportingVerificationEmailForm)

    @override_settings(DEBUG=True)
    def test_verification_post(self):
        response = self.client.post(
            self.verify_url, data={"email": "test@projectcallisto.org"}, follow=True
        )
    ...
    def test_verification_get_confirmation(self):
        ...
        self.client.get(
            reverse(
                ...
                kwargs={"uidb64": uidb64, "token": token, "uuid": self.report.uuid},
            )
        )
        self.user.account.refresh_from_db()
