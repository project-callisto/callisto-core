...
class AccountEmailParsingTest(ReportFlowTestCase):
    ...
    def test_case_lowered(self):
        BulkAccount.objects.create(emails="TECH@projectcallisto.org", site_id=2)
        self.assertEqual(
            Account.objects.filter(school_email="TECH@projectcallisto.org").count(), 0
        )
        self.assertEqual(
            Account.objects.filter(school_email="tech@projectcallisto.org").count(), 1
        )

    def test_empty_emails_not_sent(self):
        bulk_account = BulkAccount.objects.create(emails=" , ", site_id=2)

        self.assertEqual(bulk_account.parsed_emails, ["", ""])
    ...
    def test_spaces_allowed_in_email_list(self):
        bulk_account = BulkAccount.objects.create(
            emails="first@example.com,  second@example.com", site_id=2
        )

        self.assertEqual(
            bulk_account.parsed_emails, ["first@example.com", "second@example.com"]
        )

    def test_special_characters_allowed_in_email(self):
        bulk_account = BulkAccount.objects.create(
            emails="first!?#$%^&*@example.com", site_id=2
        )

        self.assertEqual(bulk_account.parsed_emails, ["first!?#$%^&*@example.com"])

    def test_basic_case(self):
        bulk_account = BulkAccount.objects.create(
            emails="first@example.com,second@example.com,third@example.com", site_id=2
        )
    ...
    def test_can_run_multiple_times_on_one_account(self):
        bulk_account = BulkAccount.objects.create(emails="first@example.com", site_id=2)
        self.assertEqual(bulk_account.parsed_emails, ["first@example.com"])
