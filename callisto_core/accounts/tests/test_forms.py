...
class StudentVerificationTest(ReportFlowTestCase):
    ...
    def test_projectcallisto_not_accepted(self):
        form = ReportingVerificationEmailForm(
            {"email": "myname@projectcallisto.org"}, school_email_domain="example.com"
        )
        self.assertFalse(form.is_valid())

    def test_subdomain_not_accepted(self):
        form = ReportingVerificationEmailForm(
            {"email": "myname@cats.example.com"}, school_email_domain="example.com"
        )
        self.assertFalse(form.is_valid())

    def test_domain_match_on_username_not_accepted(self):
        form = ReportingVerificationEmailForm(
            {"email": "example.com@gmail.com"}, school_email_domain="example.com"
        )
        self.assertFalse(form.is_valid())
    ...
    def test_control(self):
        form = ReportingVerificationEmailForm(
            {"email": "hello@example.com"}, school_email_domain="example.com"
        )
    ...
    def test_empty_domain_allowed(self):
        form = ReportingVerificationEmailForm(
            {"email": "hello@cats.com"}, school_email_domain=""
        )
        self.assertTrue(form.is_valid())
