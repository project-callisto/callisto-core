...
class FormPersistenceTest(test_base.ReportFlowHelper):
    ...
    def setUp(self):
        ...
        self.wizard_url = reverse(
            "report_update", kwargs={"step": "0", "uuid": self.uuid}
        )
    ...
    def test_when_changed_and_post(self):
        ...
        self.assertEqual(form_before, form_after)
        self.assertNotEqual(
            "this text is not persistent", form_after[0][0]["question_text"]
        )
        self.assertEqual(question_text, form_after[0][0]["question_text"])
    ...
    def test_response_forms_identical_when_form_changed(self):
        ...
        form_after = response.context["form"].serialized
        self.assertNotEqual(
            "this text is not persistent", form_after[0]["question_text"]
        )
        self.assertEqual(form_before, form_after)
...
class ViewTest(test_base.ReportFlowHelper):
    ...
    def setUp(self):
        ...
        self.data = {"question_2": "aloe ipsum speakerbox"}
        self.url = reverse(
            "report_update", kwargs={"step": self.step, "uuid": self.uuid}
        )
        ...
        self.review_url = reverse(
            "report_update",
            kwargs={"step": view_helpers.StepsHelper.done_name, "uuid": self.uuid},
        )
    ...
    def test_storage_populates_form_data(self):
        ...
        self.assertEqual(form.data["question_3"], self.data["question_3"])

    def test_review_page_textbox(self):
        self.client_post_answer_question()
        response = self.client.get(self.review_url)
        form_data = response.context["form_data"]
        self.assertIn({"eat it now???": ["blanket ipsum pillowfight"]}, form_data)
