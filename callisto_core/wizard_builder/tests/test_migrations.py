...
class QuestionPageMigrationTest(MigrationTest):
    ...
    def test_attributes_populated(self):
        OldQuestionPage = self.get_model_before("wizard_builder.QuestionPage")
        old_page = OldQuestionPage.objects.create(position=20, section=1)
...
class PopulateTypeMigrationTest(MigrationTest):
    ...
    def test_type_populated(self):
        ...
        self.assertEqual(FormQuestion.objects.get(id=formquestion.id).type, None)
        self.assertEqual(
            FormQuestion.objects.get(id=radiobutton.id).type, "radiobutton"
        )
        self.assertEqual(FormQuestion.objects.get(id=checkbox.id).type, "checkbox")
        self.assertEqual(FormQuestion.objects.get(id=textarea.id).type, "textarea")
        self.assertEqual(
...
class PopulateDropdownMigrationTest(MigrationTest):
    ...
    def test_type_populated(self):
        ...
        self.assertEqual(FormQuestion.objects.get(id=yes_question_id).is_dropdown, True)
        self.assertEqual(FormQuestion.objects.get(id=no_question_id).is_dropdown, False)
...
class MoveChoiceQuestionMigrationTest(MigrationTest):
    ...
    def test_type_populated(self):
        ...
        self.assertEqual(old_base_question._meta.model_name.lower(), "formquestion")
        self.assertEqual(new_base_question._meta.model_name.lower(), "formquestion")
        self.assertEqual(old_base_question.id, new_base_question.id)
...
class DropdownMigrationTest(MigrationTest):
    ...
    def test_type_populated(self):
        ...
        no_dropdown = FormQuestion.objects.create(type="radiobutton")
        yes_dropdown = FormQuestion.objects.create(type="radiobutton", is_dropdown=True)
