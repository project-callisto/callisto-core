...
class ManagerTest(TestCase):
    ...
    def _serialize(self, forms):
        return [form.serialized for form in forms]

    def test_accurate_number_of_forms_present(self):
        self.assertEqual(
            len(self.manager.get_form_models()), len(models.Page.objects.on_site(1))
        )
    ...
    def test_manager_persists_form_data(self):
        ...
        self.assertNotEqual(
            "this text is not persistent", form_data_after[0][0]["question_text"]
        )
    ...
    def test_only_questions_for_current_site_present(self):
        ...
        self.assertEqual(previous_questions, page.site_questions(site_id=1))
        self.assertNotEqual(previous_questions, page.site_questions(site_id=site.id))
        self.assertGreater(
            len(page.formquestion_set.all()), len(previous_all_questions)
        )
